Fuel = (function()
  ...
  function this.init(s)
    ...
    _G['rocketTankSizeTier'] = _G['rocketTankSizeTier'] / 2;
    ...
    for i=1,#elemIDs,1 do
      ...
      if kinds[elem.kind]~=nil then
        ...
        elem.size = 'xs'
        for k,o in pairs(s.tankSizes[kind]) do
          local mhp = core.getElementMaxHitPointsById(elem.uid)
          if mhp >= o[3] and mhp < o[3]*2 then elem.size = k end
        end
    ...
    KeyActions:register('tick', 'FuelUpdate', 'FuelUpdate', s, 'coUpdate');
    unit.setTimer('FuelUpdate', 0.05);
